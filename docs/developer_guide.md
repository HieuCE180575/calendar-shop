...
graph TD
    ...
    Service --> Repositories[3. Repositories - Data Access]
    Repositories --> SQL[(SQL Server Database)]
```

- **Lớp Controller (Biên giới HTTP):** Chỉ tiếp nhận HTTP Request, kiểm tra phân quyền JWT (`[Authorize]`) và trả về HTTP Status Code. **Cấm** tiêm trực tiếp `AppDbContext` hoặc `IRepository<T>` vào đây. Mọi thao tác lấy danh sách phải dùng thuộc tính `[EnableQuery]` của OData.
- **Lớp Service (Business Logic):** Chứa toàn bộ nghiệp vụ ứng dụng. **Cấm** tiêm trực tiếp `AppDbContext` vào đây; tất cả truy xuất dữ liệu phải thông qua các Repository (`IRepository<T>`). **Cấm** trả về HTTP Status Code từ Service. Nếu có lỗi, bắt buộc quăng (throw) Exception. Sử dụng AutoMapper để map thực thể DB thành DTO trước khi trả về Controller.
- **Lớp Repositories:** Chỉ làm việc trực tiếp với database thông qua các `IRepository<T>` trong thư mục `Repositories/`. Phải trả về kiểu `IQueryable<T>` để tối ưu hóa việc truy vấn trì hoãn.
...
- **Quy tắc:**
  ...
  2.  Sử dụng `.ProjectTo<TDto>(_mapper.ConfigurationProvider)` trên `IQueryable` để EF Core tự dịch thành câu lệnh SQL SELECT cột đích danh:
      ...
      {
          return _productRepository.Entities.Where(x => !x.IsDeleted)
...
- **Quy tắc:** Lưu trữ cả Access Token và Refresh Token một cách bảo mật dưới local bằng cơ chế mã hóa.
...
### 6. Tự động làm mới Token (Auto-Refresh Token Flow)

- **Quy tắc:** Interceptor của Dio bắt buộc phải cấu hình bắt lỗi `401 Unauthorized` để tự động gửi request refresh token ngầm dưới background, thực hiện lưu token mới và gửi lại (retry) request bị lỗi ban đầu để mang lại trải nghiệm không gián đoạn (Seamless UX).
- **Mẫu xử lý trong ApiClient Interceptor:**
  ```dart
  onError: (error, handler) async {
    if (error.response?.statusCode == 401) {
      final options = error.requestOptions;
      // ...
      // Gọi POST /api/Auth/refresh với Access Token và Refresh Token cũ
      // Nếu thành công, lưu token mới và thực hiện retry:
...
### 7. Tạo State & Providers bằng Riverpod Generator

- **Quy tắc:** Sử dụng các annotation `@riverpod` hoặc `@Riverpod(keepAlive: true)` để Riverpod Generator tự động biên dịch và sinh code an toàn hơn.
- **Khai báo Core Providers (`core_providers.dart`):**
  ```dart
...
- **Công cụ:** Sử dụng **xUnit**, **Moq** (để giả lập các tầng phụ thuộc như Repository/UnitOfWork), và **FluentAssertions** (để viết mã assertion dễ đọc).
...
- **Ví dụ mã kiểm thử Service:**
  ...
  public class ProductServiceTests
  {
      ...
      public async Task GetProductById_WhenProductExists_ReturnsProductDto()
      {
          ...
          // Giả lập Repository truy vấn qua Entities
          var productsList = new List<Product> { product }.AsQueryable();
          _mockProductRepository.Setup(r => r.Entities).Returns(productsList);
